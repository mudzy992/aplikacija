...
export class ArticleController {
  ...
  @UseInterceptors // Prihvata rad sa presretačima
  (
    FileInterceptor('photo', { // te koristi FIleUploadPresretač (FileIntercepto), gdje uzima iz photo vrijednosti
      // dest: StorageConfig.photos // Najjednostavniji način (definisati destinaciji u tjt.)
    storage: diskStorage ({ // i tu fotografiju pokušava da sačuva na diskStorage
      destination: StorageConfig.photoDestination, // na lokaciji koja je definisana u StorageConfig
      filename: (req, file, callback)=>{ // filename se formira tako što mi ručno pravimo funkciju koja će formirati filename
        let original: string = file.originalname;
        let normalized = original.replace(/\s+/g, '-') // u slučaju da se pojavi whitespace /\s+ globalno ga zamjeniti /g, sa '-'
        normalized = normalized.replace(/[^A-z0-9\.\-]/g, '');
        let sada = new Date(); // 'sada' je novi datum
        ...
        let randomPart: string = new Array(10) // kreiramo niz od radnom 10 brojeva
    ...
    fileFilter: (req, file, callback) => { // prihvatamo file, fileFilter zahtjeva true/false
      // provjera fajlova
      // 1. Provjera extenzije jpg, png
      if(!file.originalname.toLowerCase().match(/\.(jpg|png)$/)){ // provjeravamo da li u svom orginalnom imenu ima jpg|png na kraju $
  ...
  async uploadPhoto(@Param('id') articleId: number, @UploadedFile() photo): Promise<ApiResponse | Photo> {
    ...
    // Koji u sebi sadrži adticleId i imagePath
     ...
     if(!savedPhoto){ // ili da nije sačuvana slika
